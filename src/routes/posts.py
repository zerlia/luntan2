...
def update_post(post_id):
    try:
        ...
        if post.user_id != request.current_user.id:
            return jsonify({"error": "您無權編輯此帖子"}), 403
        ...
        post.last_modified_at = datetime.utcnow() # 添加一個更新時間字段
...
def delete_post(post_id):
    try:
        ...
        # 檢查當前用戶是否為帖子作者或管理員
        # 注意：這裡假設 request.current_user.role 存在且管理員角色為 'admin'
        # 如果您的用戶模型沒有 role 字段，或者管理員角色名稱不同，請調整此處
        if post.user_id != request.current_user.id and getattr(request.current_user, 'role', None) != 'admin':
            return jsonify({"error": "您無權刪除此帖子"}), 403
