...
export async function seedData(db: D1Database): Promise<void> {
  ...
  for (const code of inviteCodes) {
    await db.prepare('INSERT INTO invite_codes (code) VALUES (?)').bind(code).run();
  }

  // Insert admin accounts
  for (const admin of adminAccounts) {
    await db.prepare('INSERT INTO admin_accounts (username, password) VALUES (?, ?)').bind(admin.username, admin.password).run();
  }
